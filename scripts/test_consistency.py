#!/usr/bin/env python3
"""Quick consistency test — 3 main variants in parallel."""
...
VARIANTS = {
    "BASELINE": None,  # No dynamic sizing
    "DYN-A": [(80, 1.00), (75, 0.75), (70, 0.50), (65, 0.35)],  # Original
    "DYN-B": [(85, 1.00), (80, 0.70), (75, 0.50), (70, 0.35), (65, 0.25)],  # Aggressive
}
...
def get_exp_mult(score, tiers):
    if tiers is None: return 1.0
    for min_score, mult in tiers:
        if score >= min_score: return mult
    return 0.0
...
def backtest_coin(args):
    ...
    for idx in range(INITIAL, len(da)):
        ...
        for ent in entries:
            ...
            if not is_sh:
                ...
                if bl <= sl_p:
                    ...
                    trades.append({'t':'SL','dir':'L','yr':dt_cls.fromtimestamp(dt_val/1000,tz=timezone.utc).year}); intrabar = True
            else:
                sl_p = ep*(1+sl_pct)
                if bh >= sl_p:
                    sl_roi = ((ep-sl_p)/ep*100)*mp*tc*ent_lev/BASE
                    eq += sl_roi*rem2/100*ent_ff
                    trades.append({'t':'SL','dir':'S','yr':dt_cls.fromtimestamp(dt_val/1000,tz=timezone.utc).year}); intrabar = True
            ...
            if raw_roi <= -ent_sl:
                ...
                trades.append({'t':'SL','dir':'S' if is_sh else 'L','yr':dt_cls.fromtimestamp(dt_val/1000,tz=timezone.utc).year}); rm = True
                if is_sh:
                    consec_s += 1; cd_f = fib_bars(consec_s)
                    if cd_f > 0: cd_s_until = idx + cd_f
                else:
            ...
            if tp_s >= len(TP) and not rm:
                if not is_sh:
                    ...
                    if bl <= tstop:
                        eq += raw_roi*rem2/100*ent_ff
                        trades.append({'t':'TRAIL','dir':'L','yr':dt_cls.fromtimestamp(dt_val/1000,tz=timezone.utc).year}); rm = True; consec_l = 0
                else:
                    if tstop is None: tstop = cc*(1+tr_rate)
                    tstop = min(tstop, hi*(1+tr_rate)); ent['tstop'] = tstop
                    if bh >= tstop:
                        eq += raw_roi*rem2/100*ent_ff
                        trades.append({'t':'TRAIL','dir':'S','yr':dt_cls.fromtimestamp(dt_val/1000,tz=timezone.utc).year}); rm = True; consec_s = 0

            if is_sh and not rm and ts >= 2:
                eq += raw_roi*rem2/100*ent_ff
                trades.append({'t':'TREND_REV','dir':'S','yr':dt_cls.fromtimestamp(dt_val/1000,tz=timezone.utc).year}); rm = True
            ...
            if not is_sh and not rm and ts <= -2:
                eq += raw_roi*rem2/100*ent_ff
                trades.append({'t':'TREND_REV','dir':'L','yr':dt_cls.fromtimestamp(dt_val/1000,tz=timezone.utc).year}); rm = True
        ...
        if can_l or can_s:
            ...
            if act in ('OPEN_LONG_ENTRY_1','OPEN_SHORT_ENTRY_1'):
                ...
                exp_mult = get_exp_mult(sc, tiers)
                if exp_mult <= 0: continue
                
                mp = e_strong if sc >= ENTRY_MIN else e_weak
                mp *= pos_mult * exp_mult
    ...
    return (coin, dict(cagr=cagr, dd=md, slr=slr, yearly=yearly_returns))


def run_variant(variant_name, tiers):
    print(f"  {variant_name}: running...", flush=True)
    tasks = [(coin, tiers) for coin in ["ETH", "BNB", "TRX"]]
    
    with concurrent.futures.ThreadPoolExecutor(max_workers=3) as executor:
        results = list(executor.map(backtest_coin, tasks))
    
    coin_results = {r[0]: r[1] for r in results if r is not None}
    
    avg_dd = mean(coin_results[c]['dd'] for c in ["ETH","BNB","TRX"])
    avg_cagr = mean(coin_results[c]['cagr'] for c in ["ETH","BNB","TRX"])
    avg_slr = mean(coin_results[c]['slr'] for c in ["ETH","BNB","TRX"])
    
    yearly_stds = []
    for coin in ["ETH","BNB","TRX"]:
        yr_rets = [coin_results[coin]['yearly'][y] for y in range(2021, 2026)]
        yearly_stds.append(stdev(yr_rets))
    avg_std = mean(yearly_stds)
    
    return variant_name, coin_results, avg_dd, avg_cagr, avg_slr, avg_std


# ── Main ──────────────────────────────────────────────────────────────
print("=" * 95)
print("  DYNAMIC SIZING CONSISTENCY TEST")
print("=" * 95)

all_results = []
for name, tiers in VARIANTS.items():
    result = run_variant(name, tiers)
...
for variant_name, coin_results, avg_dd, avg_cagr, avg_slr, avg_std in all_results:
    print(f"\n{variant_name}:")
    print(f"  Avg DD={avg_dd:.1f}%, CAGR={avg_cagr:+.1f}%, SLr={avg_slr:.0f}%, StdDev={avg_std:.1f}%")
    
    for coin in ["ETH","BNB","TRX"]:
        yr_str = ", ".join(f"{y}:{coin_results[coin]['yearly'][y]:+.1f}%" for y in range(2021,2026))
        yr_list = [coin_results[coin]['yearly'][y] for y in range(2021, 2026)]
        coin_std = stdev(yr_list)
        print(f"    {coin}: {yr_str} | std={coin_std:.1f}%")

print("\n" + "=" * 95)
print("  RANKING (lowest StdDev = best consistency)")
print("=" * 95)

ranked = sorted(all_results, key=lambda x: x[5])  # Sort by avg_std
for i, (name, _, _, cagr, _, std) in enumerate(ranked, 1):
    print(f"  {i}. {name}: StdDev={std:.1f}%, CAGR={cagr:+.1f}%")

best_name = ranked[0][0]
best_std = ranked[0][5]
best_cagr = ranked[0][3]
print(f"\n★ BEST FOR CONSISTENCY: {best_name} (StdDev={best_std:.1f}%, CAGR={best_cagr:+.1f}%)")
