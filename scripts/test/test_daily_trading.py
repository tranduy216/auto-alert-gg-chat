...
check("ENTRY_COOLDOWN_HOURS = 12", ENTRY_COOLDOWN_HOURS == 12)
...
# Simulate old entry (> 12h ago) to test cooldown expiry
old_ts = (datetime.datetime.now() - datetime.timedelta(hours=13)).timestamp()
today = datetime.datetime.now().strftime('%Y-%m-%d')
set_state(f"{test_coin}_daily", {'date': today, 'count': 1, 'last_entry_ts': old_ts})
check("13h old entry → not in cooldown", not is_in_cooldown(test_coin))
check("13h old entry → cooldown remaining = 0", cooldown_remaining(test_coin) == 0)
